...
TH1F *leptonJetTimeHist;
TH1F *leptonJetDeltaTimeHist;
TH1F *leptonJetLongWidthHist;
TH1F *leptonJetLongWidthSigmaHist;
...
void initialize_histograms() {
    ...
    leptonJetTimeHist = new TH1F("leptonJetTime", "Reconstructed Lepton Jet Time", bins, min_range, max_range);
    leptonJetTimeHist->GetXaxis()->SetTitle("Reconstructed Time [ps]");
    leptonJetTimeHist->GetYaxis()->SetTitle("Lepton Jets");
    
    leptonJetDeltaTimeHist = new TH1F("leptonJetDeltaTime", "Delta t0 (Lepton Jet Level)", bins, min_range, max_range);
    leptonJetDeltaTimeHist->GetXaxis()->SetTitle("Delta t0 [ps]");
    leptonJetDeltaTimeHist->GetYaxis()->SetTitle("Lepton Jets");
    
    leptonJetLongWidthHist = new TH1F("leptonJetLongWidth", "Longitudinal Width of Lepton Jets", 600, 1000, 5700);
    leptonJetLongWidthHist->GetXaxis()->SetTitle("Longitudinal Width [mm]");
    leptonJetLongWidthHist->GetYaxis()->SetTitle("Lepton Jets");
    
    leptonJetLongWidthSigmaHist = new TH1F("leptonJetLongWidthSigma", "Sigma of Lepton Jet Longitudinal Energy Distribution", 1000, 0, 3000);
    leptonJetLongWidthSigmaHist->GetXaxis()->SetTitle("Longitudinal Width Sigma [mm]");
    leptonJetLongWidthSigmaHist->GetYaxis()->SetTitle("Lepton Jets");
}
...
void process_file(const std::string &filename, float energyThreshold = 1.0, float jetPtMin = 30.0, float jetPtMax = 1000.0,
                  ...
                  float leptonPtCut = 5.0, float leptonMatchDeltaR = 0.3) {
    ...
    std::vector<float> *truthPartPt = nullptr;
    std::vector<float> *truthPartEta = nullptr;
    std::vector<float> *truthPartPhi = nullptr;
    std::vector<float> *truthPartPdgId = nullptr;
    ...
    tree->SetBranchAddress("TruthPart_pt", &truthPartPt);
    tree->SetBranchAddress("TruthPart_eta", &truthPartEta);
    tree->SetBranchAddress("TruthPart_phi", &truthPartPhi);
    tree->SetBranchAddress("TruthPart_pdgId", &truthPartPdgId);
    ...
    for (Long64_t entry = 0; entry < nEntries; ++entry) {
        ...
        std::vector<std::pair<float, float>> electrons;
        for (size_t i = 0; i < truthPartPdgId->size(); ++i) {
            if ((std::abs(truthPartPdgId->at(i)) == 11) && truthPartPt->at(i) > leptonPtCut) {
                electrons.push_back(std::make_pair(truthPartEta->at(i), truthPartPhi->at(i)));
            }
        }
                ...
                if (jet_total_energy[jetIdx] > 0) {
                    ...
                    if (passEM1Cut && passEM12Cut && passLongWidthCut && passLongWidthSigmaCut && jet_weight_sum[jetIdx] > 0) {
                        ...
                        for (const auto& electron : electrons) {
                            float electronEta = electron.first;
                            float electronPhi = electron.second;
                            
                            float dEta = jetEta - electronEta;
                            float dPhi = jetPhi - electronPhi;
                        ...
                        if (isLeptonJet) {
                            leptonJetTimeHist->Fill(jet_time);
                            leptonJetDeltaTimeHist->Fill(delta_jet_time);
                            leptonJetLongWidthHist->Fill(longitudinal_width);
                            leptonJetLongWidthSigmaHist->Fill(saved_sigma);
                        } else {
...
void processmu200_jetmatching_reco(float energyThreshold = 1.0, int startIndex = 1, int endIndex = 46, 
    ...
    for (int i = startIndex; i <= endIndex; ++i) {
        ...
        if (std::filesystem::exists(filename.str())) {
            process_file(filename.str(), energyThreshold, jetPtMin, jetPtMax, deltaRThreshold, 
                        maxJets, jetWidthMin, jetWidthMax, jetEtaCut, jetEM1FractionCut, jetEM12FractionCut,
                        jetLongWidthCut, jetLongWidthSigmaCut, leptonPtCut, leptonMatchDeltaR);
        } else {
    ...
    outputFilename << "_LepPt" << std::setprecision(1) << leptonPtCut;
    outputFilename << "_LepDR" << std::setprecision(1) << leptonMatchDeltaR;
    outputFilename << ".root";
    ...
    leptonJetTimeHist->Write();
    leptonJetDeltaTimeHist->Write();
    leptonJetLongWidthHist->Write();
    leptonJetLongWidthSigmaHist->Write();
    ...
    delete leptonJetTimeHist;
    delete leptonJetDeltaTimeHist;
    delete leptonJetLongWidthHist;
    delete leptonJetLongWidthSigmaHist;
    ...
    std::cout << "  Lepton pT cut: > " << leptonPtCut << " GeV" << std::endl;
    std::cout << "  Lepton-jet matching deltaR: < " << leptonMatchDeltaR << std::endl;
}
